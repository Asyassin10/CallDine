...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const forwarded = new Headers({ "Content-Type": response.headers.get("Content-Type") ?? "audio/webm" });
  for (const name of ["accept-ranges", "content-length", "content-range"]) if (response.headers.get(name)) forwarded.set(name, response.headers.get(name)!);
