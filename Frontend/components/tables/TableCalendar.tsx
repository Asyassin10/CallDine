...
function reservationDate(when: string) { return new Date(when.replace(" · ", " ")); }
function tableId(table: string) { return table.split(" ")[0]; }
function timeSlot(when: string) { const time = when.match(/\d{1,2}:\d{2}/)?.[0]; return time ? `${time.split(":")[0].padStart(2, "0")}:00` : ""; }
...
export function TableCalendar() {
  ...
  const bookingFor = (table: string, day: Date, hour?: string) => {
    return reservations.find(item => {
      return tableId(item.table) === table && reservationDate(item.when).toDateString() === day.toDateString() && (!hour || timeSlot(item.when) === hour);
    });
  };

  return <section className={styles.calendar}><header className={styles.controls}><div><p className={styles.eyebrow}>Dining room availability</p><h2>{title}</h2></div><div className={styles.actions}><button className="button" onClick={() => move(-1)}>← Previous</button><button className="button" onClick={() => setDate(new Date())}>Today</button><button className="button" onClick={() => move(1)}>Next →</button><div className={styles.viewToggle}><button className={view === "daily" ? styles.selected : ""} onClick={() => setView("daily")}>Daily</button><button className={view === "weekly" ? styles.selected : ""} onClick={() => setView("weekly")}>Weekly</button></div></div></header><div className={styles.legend}><span><i className={styles.free} />Available</span><span><i className={styles.booked} />Booked</span></div><div className={styles.scroll}><div className={`${styles.grid} ${view === "daily" ? styles.daily : styles.weekly}`}><div className={styles.corner}>TABLE</div>{view === "daily" ? hours.map(hour => <div className={styles.columnHead} key={hour}>{hour}</div>) : days.map(day => <div className={styles.columnHead} key={day.toISOString()}><strong>{dayName.format(day)}</strong><span>{dateName.format(day)}</span></div>)}{tables.map(table => <div className={styles.row} key={table}><strong className={styles.tableName}>{table}</strong>{view === "daily" ? hours.map(hour => { const booking = bookingFor(table, date, hour); return <div className={`${styles.cell} ${booking ? styles.booked : styles.free}`} key={hour}>{booking && <button onClick={() => setSelected(booking)}>{booking.guest}<small>{timeSlot(booking.when)}</small></button>}</div>; }) : days.map(day => { const booking = bookingFor(table, day); return <div className={`${styles.cell} ${booking ? styles.booked : styles.free}`} key={day.toISOString()}>{booking && <button onClick={() => setSelected(booking)}>{booking.guest}<small>{timeSlot(booking.when)}</small></button>}</div>; })}</div>)}</div></div>{selected && <div className={styles.backdrop} onClick={() => setSelected(null)}><section className={styles.modal} onClick={event => event.stopPropagation()}><div className={styles.modalHead}><div><p className={styles.eyebrow}>Reservation #{selected.id}</p><h3>{selected.table}</h3></div><button className="button" onClick={() => setSelected(null)}>Close</button></div><dl><div><dt>Guest</dt><dd>{selected.guest}</dd></div><div><dt>Time</dt><dd>{selected.when}</dd></div><div><dt>Party size</dt><dd>{selected.guests} guests</dd></div><div><dt>Status</dt><dd>{selected.status}</dd></div></dl></section></div>}</section>;
}
