...
export function VoiceAssistant() {
  ...
  const silenceTimer = useRef<number | null>(null), sound = useRef<AudioContext | null>(null), capture = useRef<AudioContext | null>(null), processor = useRef<ScriptProcessorNode | null>(null), samples = useRef<Float32Array[]>([]);
  useEffect(() => () => { void end(); }, []);
  ...
  async function start() {
    ...
    callRecorder.current.start(1000);
  ...
  async function playSpeech(text: string) {
    ...
    if (!response.body || !MediaSource.isTypeSupported("audio/mpeg")) { audio.current.src = URL.createObjectURL(await response.blob()); audio.current.onended = listen; return audio.current.play(); }
    const media = new MediaSource(), url = URL.createObjectURL(media); audio.current.src = url; audio.current.onended = () => { URL.revokeObjectURL(url); listen(); };
  ...
  function startProcessingSound() { sound.current = new AudioContext(); const tone = sound.current.createOscillator(), gain = sound.current.createGain(); tone.frequency.value = 190; gain.gain.value = .015; tone.connect(gain).connect(sound.current.destination); tone.start(); }
  function stopProcessingSound() { sound.current?.close(); sound.current = null; }
  async function saveRecording() {
    ...
    if (!conversation.current || !recording.current.length) return;
    const form = new FormData(); form.append("file", new Blob(recording.current, { type: "audio/webm" }), "call.webm");
  ...
  async function end() {
    ...
    let saved = true; try { await saveRecording(); } catch { saved = false; }
    mixContext.current?.close(); meeting.current?.audioVideo.stop(); stream.current?.getTracks().forEach(track => track.stop()); setState("ready"); setText(saved ? "Call ended and recording saved." : "Call ended, but its recording could not be saved.");
  }
  ...
  return <div className={styles.page}><div className={styles.card}><div className={styles.status}><i className={styles.statusDot}/><span>{label.toUpperCase()}</span></div><div className={styles.orb}><i className={`${styles.halo} ${state !== "listening" ? styles.haloQuiet : ""}`}/><i className={`${styles.halo} ${styles.haloSecond} ${state !== "listening" ? styles.haloQuiet : ""}`}/><div className={styles.logoOrb}>CD</div></div><div className={styles.copy}><div className={styles.title}>{state === "ready" ? "Start a voice call" : label}</div><div className={styles.description}>{text}</div></div><div className={styles.waveform}>{Array.from({ length: 32 }, (_, i) => <i key={i} className={`${styles.bar} ${styles[state === "ready" ? "connecting" : state]}`} style={{ height: 12 + (i % 7) * 7, animationDelay: `${i * 55}ms` }}/>)}</div><div className={styles.actions}>{state === "ready" ? <button className={styles.start} onClick={start}>Start voice call</button> : <><button className={styles.secondary} onClick={toggleMute}>{muted ? "Unmute" : "Mute"}</button><Link href="/app/chat" className={styles.secondary}>Chat</Link><button className={styles.end} onClick={end}>End</button></>}</div></div></div>;
}
