...
const adminNav = [["Overview","/admin"],["Orders","/admin/orders"],["Reservations","/admin/reservations"],["Menu","/admin/menu"],["AI Calls","/admin/calls"],["Recommendations","/admin/recommendations"],["Customers","/admin/customers"],["Settings","/admin/settings"]];
...
export function AppShell({role,children}:{role:SessionRole;children:React.ReactNode}){
  ...
  return <div className={`${styles.shell} ${pathname==="/app/ai"?styles.voiceShell:""}`}>
    ...
    <aside className={`${styles.sidebar} ${open?styles.open:""}`}><div className={styles.brand}><Logo/><small>{role==="admin"?"Restaurant Admin":"Guest app"}</small></div>
      <nav className={styles.nav}>{nav.map(([label,href],index)=><div key={href}>{role==="admin"&&[0,4,8].includes(index)&&<div className={styles.group}>{index===0?"OPERATIONS":index===4?"AI":"ACCOUNT"}</div>}<Link onClick={()=>setOpen(false)} className={`${styles.link} ${active(href)?styles.active:""}`} href={href}>{label}</Link></div>)}</nav>
