...
type ApiMenuItem = {id:number;name:string;category:MenuCategory;price:number;image_url:string;available:boolean};

export async function getMenu(): Promise<MenuItem[]> {
  const response = await fetch(`${backend}/api/v1/menu`, { cache:"no-store" });
  const items = await response.json() as ApiMenuItem[];
  return items.map(item => ({ id:String(item.id), name:item.name, slug:slugify(item.name), category:item.category, price:item.price, image:item.image_url, available:item.available }));
}
