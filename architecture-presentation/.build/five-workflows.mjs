...
function overview(){
 ...
 function box(x,y,w,h,title,sub='',icon=null,planned=false){const a=rect(s,x,y,w,h,c.paper,planned?c.purple:c.line,planned);let tx=x+20,tw=w-40;
  ...
  const ty=y+(icon?82:18);txt(s,title,tx,ty,tw,68,w<400?26:28,c.ink,true);if(sub)txt(s,sub,x+20,ty+(h<200?50:70),w-40,Math.max(28,h-(ty-y)-54),23,c.muted);return a;}
...
],'','This pipeline prepares restaurant knowledge. FastAPI coordinates S3, Textract, text chunking, Titan embeddings and Qdrant. The search-knowledge tool later uses these stored chunks for retrieval-augmented generation. Background processing runs inside the FastAPI application.','backend/app/services/knowledge_service.py; backend/app/services/embedding_service.py');
{
 ...
 const ds=[['Customer speaks','The browser captures the customer’s microphone audio and finishes the turn after a short silence.','calldine'],['Upload to FastAPI','Next.js forwards the completed audio turn to the FastAPI voice service.','fastapi'],['Transcribe to text','FastAPI sends the audio to Amazon Transcribe Streaming, which produces the transcript.','transcribe'],['Send transcript to chat','The transcript returns to the browser, which submits it to FastAPI Chat Service.','next'],['Bedrock + tools\nOptional Guardrails','Chat Service sends conversation context to Bedrock and executes any requested business tools.','bedrock'],['Return answer text','FastAPI returns a cleaned answer to the browser. The browser then requests speech.','fastapi'],['Polly creates speech','FastAPI sends the answer to Polly and returns MP3 audio through Next.js.','polly'],['Customer hears reply','The browser plays the reply, then resumes listening.','calldine']];
 ...
 txt(s,'Bedrock Guardrails: optional prompt-attack, harmful-content and word filters.',60,940,1720,45,26,c.accent,true);
 txt(s,'Turn-based audio. Chime media is separate from the transcription path.',60,991,1700,42,24,c.muted);
...
{
 const s=base(detailed,'5. Bedrock Guardrails workflow','Optional policy on Bedrock chat calls, including voice transcripts',5);
 const a=node(s,{x:60,y:335,w:490,h:200,title:'1. Chat or voice transcript',desc:'FastAPI prepares the conversation',icon:'fastapi'});
 const b=node(s,{x:715,y:335,w:490,h:200,title:'2. Bedrock + Guardrails',desc:'Attach configured policy ID and version',icon:'bedrock',stroke:c.accent});
 edge(s,a,b);
 const ok=node(s,{x:1370,y:335,w:490,h:200,title:'3A. Allowed',desc:'Continue model and tool workflow',icon:'bedrock',stroke:c.green});
 const no=node(s,{x:715,y:680,w:490,h:200,title:'3B. Policy intervenes',desc:'Return a blocked response',icon:'bedrock',stroke:c.accent});
 const end=node(s,{x:1370,y:680,w:490,h:200,title:'4. Return response',desc:'Display text; voice replies use Polly',icon:'polly'});
 edge(s,b,ok);edge(s,b,no,'bottom','top');edge(s,ok,end,'bottom','top');edge(s,no,end);
 txt(s,'Configured policy filters',60,650,590,55,32,c.accent,true);
 txt(s,'Prompt injection / jailbreaks\nHarmful or toxic content\nBlocked words',60,715,590,160,30,c.ink);
 txt(s,'Policy configuration determines coverage. Deployed AWS policy has not been verified.',60,947,1740,65,27,c.muted);
 s.speakerNotes.textFrame.setText('FastAPI receives chat text or a voice transcript. When a Guardrails ID and version are configured, it attaches the policy to Bedrock chat calls. Allowed calls continue through the model and tools. If the policy intervenes, the application handles the blocked response. The response returns to the user as text; the voice interface requests Polly audio. Policy coverage depends on the AWS configuration, which has not been verified.\n'+sources);
}
await save(detailed,'CallDine-Five-Workflows-Guardrails');
