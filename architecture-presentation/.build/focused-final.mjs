...
function overview(){const p=Presentation.create({slideSize:{width:1920,height:1080}});const s=base(p,'Complete system architecture','Numbered steps show the conversation. Arrows show requests and returned results.',1);img(s,'aws',1530,12,330,145);
 txt(s,'APPLICATION RUNTIME',60,299,690,42,24,c.accent,true);txt(s,'AWS MANAGED SERVICES',1320,292,540,42,24,c.accent,true);
 const ui=node(s,{x:60,y:350,w:275,h:215,title:'Browser',desc:'1. Text or voice input\n7. Read or hear reply',icon:'calldine'});
 const next=node(s,{x:385,y:350,w:290,h:215,title:'Next.js',desc:'Forward requests\nReturn text and audio',icon:'next'});
 const api=node(s,{x:755,y:350,w:390,h:215,title:'FastAPI',desc:'2. Authenticate + route\n6. Return text or audio',icon:'fastapi'});
 fullEdge(s,ui,next);fullEdge(s,next,api);
 const chat=node(s,{x:755,y:620,w:390,h:135,title:'4. Execute tools',desc:'RAG, orders and reservations'});fullEdge(s,api,chat,'bottom','top');
 const q=node(s,{x:385,y:620,w:290,h:135,title:'Qdrant in Docker',desc:'Knowledge vectors'});fullEdge(s,chat,q,'left','right',false,c.teal);
 const repo=node(s,{x:755,y:800,w:390,h:130,title:'Repositories',desc:'5. Read / save records'});fullEdge(s,chat,repo,'bottom','top');
 const db=node(s,{x:385,y:800,w:290,h:130,title:'SQLite',desc:'Operational records'});fullEdge(s,repo,db,'left','right');
 const ch=node(s,{x:60,y:620,w:275,h:190,title:'Chime SDK',desc:'Meeting media\nAPI session setup',icon:'chime'});fullEdge(s,ui,ch,'bottom','top');
 const defs=[['bedrock','Bedrock','3. Select tools / generate answer'],['s3','Amazon S3','Store PDFs, images and recordings'],['textract','Amazon Textract','Extract text for the knowledge pipeline'],['transcribe','Amazon Transcribe','Convert uploaded speech into text'],['polly','Amazon Polly','Convert the answer into speech']];
 ...
 const bus=rect(s,1230,397,2,476,c.accent,c.accent);fullEdge(s,api,bus,'right','left');aws.forEach(a=>fullEdge(s,bus,a,'right','left'));
...
await save(detailed,'CallDine-Focused-Workflows');
await save(overview(),'CallDine-Complete-Workflow');
