...
function overview(){
 ...
 function box(x,y,w,h,title,sub='',icon=null,planned=false){const a=rect(s,x,y,w,h,c.paper,planned?c.purple:c.line,planned);let tx=x+20,tw=w-40;
  ...
  const ty=y+(icon?82:18);txt(s,title,tx,ty,tw,68,w<400?26:31,c.ink,true);if(sub)txt(s,sub,x+20,ty+66,w-40,Math.max(48,h-(ty-y)-70),25,c.muted);return a;}
...
await save(detailed,'CallDine-Workflows-Embedding');
await save(overview(),'CallDine-Comprehensive-Architecture');
